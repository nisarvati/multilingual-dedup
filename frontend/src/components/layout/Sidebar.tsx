...
export const Sidebar = ({
  ...
  return (
    ...
    >
      <div className={cn("flex h-14 items-center border-b border-border/60", collapsed ? "justify-center px-2" : "px-4")}>
        <div className={cn("flex items-center", collapsed ? "justify-center" : "min-w-0 flex-1 gap-2.5")}>
          <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-gradient-primary shadow-glow">
            <Sparkles className="h-4 w-4 text-primary-foreground" />
          </div>
          {!collapsed && <div className="truncate text-sm font-semibold tracking-tight">Dedupe.AI</div>}
        </div>
        <Button
          variant="ghost"
          size="icon"
          className={cn("h-8 w-8 text-subtle hover:text-foreground", collapsed && "absolute right-2 top-3")}
